...
export const getServerSideProps = async (context: GetServerSidePropsContext) => {
  ...
  // No orgs in cal.diy, so there is never a valid org domain
  ...
  const [eventHostsUserData, crmResult, booking] = await Promise.all([
    ...
    // CRM-based host lookup removed with the EE teams code; crm* URL params still pass through below
    Promise.resolve(
...
const getTeamWithEventsData = async (
  teamSlug: string,
  meetingSlug: string,
  _isValidOrgDomain: boolean,
  _currentOrgDomain: string | null
) => {
